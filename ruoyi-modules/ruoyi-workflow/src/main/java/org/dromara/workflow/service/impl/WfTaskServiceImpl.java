...
public class WfTaskServiceImpl implements IWfTaskService {
    ...
    public void delegateTask(WfTaskBo bo) {
        ...
        StringBuilder commentBuilder = new StringBuilder(LoginHelper.getRealName())
            .append("->");
        String realName = userService.selectRealNameById(Long.parseLong(bo.getUserId()));
        if (StringUtils.isNotBlank(realName)) {
            commentBuilder.append(realName);
        } else {
    ...
    public void revokeProcess(WfTaskBo taskBo) {
        ...
        for (Task task : activateTaskList) {
            ...
            if (CollUtil.contains(nextUserTaskKeys, task.getTaskDefinitionKey())) {
                ...
                taskService.addComment(task.getId(), task.getProcessInstanceId(), FlowComment.REVOKE.getType(), LoginHelper.getRealName() + "撤回流程审批");
        ...
        if (CollUtil.isNotEmpty(tasks)) {
            ...
            for (Task task : tasks) {
                if (StrUtil.equals(task.getAssignee(), userIdStr)) {
                    taskService.addComment(task.getId(), processInstance.getProcessInstanceId(), FlowComment.NORMAL.getType(), LoginHelper.getRealName() + "发起流程申请");
